...
from binancer.common import MakeDirty, lock_instance, format_stacktrace
...
l = logging.getLogger("zzz")

API_LOGIN = 'rFWHu95rQM5qouuHbpzxIpv8PmAIT9DmwQS11XWTHLWgqF84OHImAFTPfZRjs0HE'
API_KEY = '54fU8VBvq1lgQkCZyK13cSmfXLhlIao4VEPLOV9x1UwVBaNvw1YYKYsN8QoFwmH8'
...
def connect() -> None:
    logger.debug(f'connecting to binance')
...
def disconnect() -> None:
    for client in clients.values():  # type: AccountClient
        if hasattr(client, 'bm'):
            logger.debug(f'Disconnecting {client} from binance...')
            client.bm.close()


def init_client() -> BinanceClient:
    global binance_client, quotes_maker
    try:
        binance_client = BinanceClient(api_key=API_LOGIN, api_secret=API_KEY)
        logger.debug("BinanceClient initialised: binance_client")
        return binance_client
    except Exception as e:
        stacktrace = format_stacktrace()
        logger.exception(f'ERROR binance client is dead {e} {stacktrace}')
        # return None


def init_quotes_maker() -> None:
    global quotes_maker
    bclient = init_client()
    try:
        logger.debug("Initialising BinanceSocketManager")
        quotes_maker = BinanceSocketManager(bclient)
        quotes_maker.start_ticker_socket(process_multi_ticker)
        subscribe_tickers()
        # subscribe_symbol('LTUSDT')
    except Exception as e:
        # traceback.print_exc()
        stacktrace = format_stacktrace()
        logger.exception(f'ERROR init quotes_maker {e} {stacktrace}')
        return
...
class AccountClient:
    ...
    def __init__(self, account: Account, client: BinanceClient):
        # from tools.logger import l
        # logger.debug('Initializing AccountClient')
        # global quotes_maker
        self.account: Account = account
        self.client: BinanceClient = client
        if not account.is_demo and account.api_key and account.secret_key:
            bm = BinanceSocketManager(client)
            self.bm: BinanceSocketManager = bm
            try:
                ...
                """
                bm.start()
                logger.debug(f'BinanceSocketManager connected started')
    ...
    def reconnect(self) -> None:
        logger.debug('Reconnecting AccountClient')
        # global quotes_maker
        if not self.account.is_demo and self.account.api_key and self.account.secret_key:
            with self.lock:
                ...
                binancer_client.clients[self.account.id] = BinanceClient(self.account.api_key, self.account.secret_key,
    ...
    def process_tickers(self, symbol_name=None, new_signal=False) -> None:
        global l
        ...
        # logger.debug('process_tickers AccountClient')
        # if task_queue:
        #    task_queue.join()
        if new_signal:
            logger.debug('Aquire lock, new_signal')

        now = datetime.now()

        with self.lock:
            if new_signal:
                logger.debug('cycle, new_signal')
            for order in self.orders:  # type: NOrder
                if order.mode == TradingMode.Stop or new_signal and order.mode != TradingMode.BuyReady:
                    # logger.debug(f'order.mode == {TradingMode.Stop}')
                    continue
                if symbol_name and order.symbol != symbol_name:
                    logger.debug(f'{order.symbol} != {symbol_name}')
                    continue
                ...
                if hasattr(order, 'order') and order.order:
                    if not hasattr(order.order, 'orderspecial'):
                        logger.debug('OrderSpecial')
                        order.order.orderspecial = OrderSpecial.objects.create(order=order.order, max_price=bid,
                                                                               min_ask_price=ask,
                                                                               max_price_date=datetime.now(),
                                                                               min_ask_date=datetime.now())
                    else:
                ...
                if order.mode == TradingMode.PendingBuy:
                    # logger.debug(f'order.mode == {TradingMode.PendingBuy}')
                    if order.order.order_kind == 0 and ask <= order.open_price or order.order.order_kind == 1 \
                        ...
                        info(f'READY #{order.order.id} to be opened: {ask} | {order.open_price}')
                        order.mode = TradingMode.BuyReady
                    elif order.order.preopen_stop_time and now >= order.order.preopen_stop_time:
                        info(f'TIMEOUT of #{order.order.id} | {order.order.preopen_stop_time}. Deleting...')
                ...
                if order.mode == TradingMode.BuyReady:
                    logger.debug(f'order.mode == {TradingMode.BuyReady}')
                    order.mode = TradingMode.Stop
                    ...
                    if ask > order.open_price and not hasattr(order, "order") and (
                            ask - order.open_price) * 100 / ask > order.strategy.corridor:
                        warning(
                            f'невозможно открыть ордер за границами коридора {cfloat(order.open_price)} < {cfloat(ask)}')
                        # order.mode = TradingMode.Stop
                        continue
                    ...
                    if not order.strategy or order.strategy.balance_limit_mode == 0 or not look_limits.exists():
                        if not self.account.is_demo:
                            logger.debug(f'> get account {self.account.name}')
                    ...
                    if order.strategy:
                        if not self.account.balance_control:
                            ...
                            trade_balance = (total_balance[0] + total_balance[1]) * float(
                                order.strategy.lot_amount) / 100
                    ...
                    if quantity < symbol.min_lot:
                        error(
                            f'количество монет покупки меньше допустимого минимума {cfloat(quantity)} < {cfloat(symbol.min_lot)}'
                    ...
                    if trade_balance < symbol.min_notional:
                        error(
                            f'объём ордера меньше допустимого минимума {cfloat(trade_balance)} < {cfloat(symbol.min_notional)}')
                        # order.mode = TradingMode.Stop
                        continue

                    if self.account.balance_control and trade_balance > balance:
                        error(
                            f'not enough money to buy an order {cfloat(trade_balance)} > {cfloat(balance)}'.capitalize())
                        # order.mode = TradingMode.Stop
                        continue
                    ...
                    if not hasattr(order, 'order'):
                        ...
                        if ask - stop_loss <= symbol.tick_size:
                            error(
                                f'уровень стоп лосс слишком близок к цене открытия {cfloat(ask - stop_loss)} <= {cfloat(symbol.tick_size)}')
                            order.mode = TradingMode.Stop
                            continue
                    ...
                    if not hasattr(order, 'order'):
                        ...
                        autotrade_levels = [{'price': round_tick(ask * (100 - level.percent) / 100),
                                             'quantity': round_lot(
                                                 level.lot / round_tick(ask * (100 - level.percent) / 100)),
                    ...
                    for level in stop_levels:
                        if level['sl_delta'] is not None:
                            if level['sl_delta'] <= symbol.tick_size:
                                error(
                                    f'разница в цене для скользящего стоп-лосса меньше допустимой {cfloat(level["sl_delta"])} <= {cfloat(symbol.tick_size)}')
                                ok = False
                                break
                    ...
                    if not hasattr(order, 'order'):
                        ...
                        morder.save()
                        morder.orderspecial = OrderSpecial.objects.create(order=morder, max_price=bid,
                                                                          min_ask_price=ask,
                                                                          max_price_date=datetime.now(),
                                                                          min_ask_date=datetime.now())
                        # order.order = morder
                ...
                # === Market state ===
                    ...
                    logger.debug(f'TradingMode.InMarket')
                    # logger.debug(f'order.mode == {TradingMode.InMarket}')
                    try:
                        order.order.refresh_from_db()
                    except:
                        logger.exception(f'order_id = {order.order.order_id}')
                ...
                # === Ready state ===
                    ...
                    # logger.debug(f'order.mode == {TradingMode.Ready}')
                    # check stop-loss
                    need_stop = False
                    if order.stoploss_price and ask <= order.stoploss_price:
                        info(
                            f'CLOSED by SL ({order.order.get_stoploss_type_display()}) {cfloat(order.stoploss_price)} #{order.order.id}',
                            notify=NotifyType.Closed, sl=True)
                        need_stop = True
                    elif order.pending_stop_time and now >= order.pending_stop_time:
                        info(f'CLOSED order by time-out',
                             notify=NotifyType.Closed, sl=True)
                        need_stop = True
                    if need_stop:
                        morder = order.order
                    ...
                    if order.trailing_level and bid >= order.trailing_level:
                        ...
                        with lock_instance(morder) as morder:
                            with MakeDirty(morder):
                                level = next((lvl for lvl in morder.takeprofit_levels if
                                              'filled' not in lvl and not lvl.get('hidden')), None)
                                if not level:
                    ...
                    if order.strategy and order.strategy.simulation or self.account.is_demo or order.tp_mode == 1:
                        if hasattr(order,
                                   "takeprofit_price") and order.takeprofit_price and bid >= order.takeprofit_price:
                            morder = order.order
                            morder.refresh_from_db()
                            levels_count = sum(
                                1 for lvl in morder.takeprofit_levels if 'filled' not in lvl and not lvl.get('hidden'))

                            if levels_count == 0:
                                logger.debug(f'количество уровней ордера {order.order.id} равно нулю')
                                order.mode = TradingMode.Stop
                                continue
                            ...
                            if order.takeprofit_quantity > 0:
                                ...
                                with lock_instance(morder) as morder:
                                    ...
                                    setattr(morder, "no_sync", True)
                                    morder.save()
                                info(f'сработал тейк-профит #{morder.id} {cfloat(order.takeprofit_price)}',
                                     notify=NotifyType.Closed)

                            with lock_instance(morder) as morder:
                                lvl = next((lvl for lvl in morder.takeprofit_levels if
                                            'filled' not in lvl and not lvl.get('hidden')), None)
                                lvl['filled'] = True
                                ...
                                if levels_count > 1:
                                    ...
                                    lvl = next(lvl for lvl in morder.takeprofit_levels if
                                               'filled' not in lvl and not lvl.get('hidden'))
                                    ...
                                    """
                                    if order.takeprofit_quantity > 0:
                                        info(f'по ордеру все уровни ТП достигнуты #{morder.id} {cfloat(morder.profit)}',
                                             notify=NotifyType.Closed)
                                        order.mode = TradingMode.Stop
                                    else:
                    ...
                    if hasattr(order, "takebuy_price") and order.takebuy_price and ask <= order.takebuy_price:
                        with lock_instance(order.order) as morder:
                            with MakeDirty(morder):
                                ...
                                morder.save()
                                info(f'достигнут ТБ {cfloat(lvl["price"])}', notify=NotifyType.Order,
                                     prev_tb=last_count == 1)
                        binancer_client.trade('ADD_ORDER', order.order.id,
                                              lvl['quantity'] * (lvl['average'] and -1 or 1), lvl['average'] or any(
                                l.get('hidden') for l in order.order.takeprofit_levels))
                    ...
                    if hasattr(order, 'autobuy_price') and order.autobuy_price and ask <= order.autobuy_price:
                        with lock_instance(order.order) as morder:
                            with MakeDirty(morder):
                                ...
                                morder.save()
                                info(f'достигнут auto-trade {cfloat(order.autobuy_price)}', notify=NotifyType.Order,
                                     prev_tb=last_count == 1)
                        order.mode = TradingMode.Stop
            ...
            if new_signal:
                logger.debug('> refresh order states')
    ...
    def process_trade(self, msg: dict) -> None:
        # if 'x' not in msg:
        #    return
        logger.debug(f'Processing tade AccountClient')
        ...
        try:
            ...
            logger.debug(f'process_trade: wrong order_id: {order_id}')
            return
        ...
        def error(msg, **kwargs):
            l.error(msg, strategy=order.strategy, **kwargs)
            logger.error(msg, strategy=order.strategy, **kwargs)

        def warning(msg, **kwargs):
            l.warning(msg, strategy=order.strategy, **kwargs)
            logger.warning(msg, strategy=order.strategy, **kwargs)

        def info(msg, **kwargs):
            l.info(msg, strategy=order.strategy, **kwargs)
    ...
    def add_order(self, morder) -> None:
        logger.debug(f'adding an order to the queue to track ')
        global l
        # global l
        order = NOrder(
        ...
        try:
            ...
            traceback.print_exc()
            logger.exception(f'Cant find local order order_id = {morder.id}')
            return
        ...
        if morder.symbol.name not in self.symbols:
            ...
            # logger.debug(f'adding to symbols')
        ...
        logger.debug(f'added to orders')
    ...
    def sync_orders(self) -> None:
        with self.lock:
            ...
            # reload orders info
            orders = Order.objects.filter(Q(account=self.account) &
                                          (Q(status=OrderStatus.ACTIVE) & ~Q(order_id='') & ~Q(
                                              order_id__isnull=True) & Q(expert__isnull=True)
                                           | Q(status=OrderStatus.PENDING) & Q(parent_order__isnull=True)))
            if orders:
                logger.debug(f'Found {orders.aggregate(Count("id"))["id__count"]} orders of {self.account.name} ')
            for morder in orders:  # type: Order
                self.add_order(morder)
                logger.debug('Orders synced\n')

    def sync_order(self, order) -> None:
    ...
    def open_order(self, symbol: Symbol, bot: Bot, strategy: Strategy, price: float, signal: Signal) -> None:

        logger.debug(f"> openning order " + self.account.name,
                     strategy=strategy, signal=signal)
        # subscribe to quotes
        if symbol.name not in self.symbols:
            # logger.debug(f'подписка на пару {symbol.name}')
            logger.debug("> subscribe")
            subscribe_symbol(symbol.name)
            self.symbols.append(symbol.name)

        if symbol.name not in subscriptions:
            logger.error(f'Could not open an order due to lack of subscription for {symbol.name}',
                         strategy=strategy, signal=signal)
            return
        ...
        if bot.one_order_per_symbol:
            ...
            if has_order:
                logger.warning(
                    f'Impossible to open еру order, because the symbol {symbol.name}@{self.account} is already in work',
                    strategy=strategy, signal=signal)
                return
    ...
    def smart_order(self, order: Order):
        logger.debug("Processing a smart oder")
        if order.symbol.name not in self.symbols:
            logger.info(f'Not subscribed. Subscribing to {order.symbol.name}')
            subscribe_symbol(order.symbol.name)
            self.symbols.append(order.symbol.name)

        if order.symbol.name not in subscriptions:
            logger.error(f'Could not open an order due to lack of subscription for a pair {order.symbol.name}')
            return

        with self.lock:
            norder = NOrder(TradingMode.PendingBuy if order.status == OrderStatus.PENDING else TradingMode.BuyReady,
                            order.symbol.name, None, None, None, order.open_price)
            norder.tp_mode = 1
            norder.order = order
            self.orders.append(norder)

        logger.info(f"CREATED !LOCAL! #{order.id}", order=order)

        self.process_tickers(order.symbol.name, True)


def process_ticker(msg: dict) -> None:
    global subscriptions
    # logger.debug('process_ticker')
    ...
    if not check or last_item["a"] != msg["a"] or last_item["b"] != msg["b"]:
        ...
        # logger.debug(f'_r_quotes.publish {data}')
        _r_quotes.publish('common.quotes', data)
...
def process_multi_ticker(msgs: list) -> None:
    # logger.debug(f'process_multi_ticker')
    try:
        for msg in msgs:
        ...
        for client in clients:
            # workers_queue.put(i)
            _r_quotes.rpush('jobs', client)
    except:
        logger.exception('\n'.join(traceback.format_exception(*sys.exc_info())))


def subscribe_tickers() -> None:
    # global quotes_maker, bclient
    if not quotes_maker:
        logger.critical(f'BinanceSocketManager is down. Returning.')
        return

    tickers = quotes_maker._client.get_ticker()
    for ticker in tickers:
        if ticker['symbol'] in subscriptions:
            # logger.debug(f'ticker for {ticker["symbol"]} in tickers')
            continue
        # logger.debug(f'adding ticker for {ticker["symbol"]} to tickers')

        symbols = Symbol.objects.filter(name=ticker['symbol'])
        if len(symbols) >= 2:
            logger.warning(f'{ticker["symbol"]} is {len(symbols)}')
            symbols.delete()
        ...
        if created:
            with lock_instance(sym) as sym:
                ...
                tick_size = next(
                    ...
                    0)
                if tick_size == 0:
                    logger.critical(f'tick_size {tick_size}')
                if min_lot == 0:
                    logger.critical(f'min_lot {min_lot}')
                if min_notional == 0:
                    logger.critical(f'min_notional {min_notional}')
                ...
                sym.save()
                logger.debug(f'Symbol {ticker["symbol"]} {min_notional} | {min_lot} | {tick_size} created')
        else:
            with lock_instance(sym) as sym:
                l.info(f'Symbol {ticker["symbol"]} existed')
                # logger.debug(f'Symbol {ticker["symbol"]} {min_notional} | {min_lot} | {tick_size} updated')
            pass
        quote = NQuote(
        ...
        #    logger.debug(f'{ticker["symbol"]} == {quote.min_notional} !')

        subscriptions[ticker['symbol']] = quote
        # logger.warning(f"Ticker {ticker['symbol']} subscribed")

    logger.debug(f'Tickers subscribed')


def subscribe_symbol(symbol: str) -> None:
    logger.debug(f'Subscribing to symbol: {symbol}')
    global subscriptions, quotes_maker
    if not quotes_maker:
        logger.critical(f'BinanceSocketManager (object null) для подписки на котировки. Initin the client.')
        init_quotes_maker()
        # return
    if symbol not in subscriptions and symbol != '':
        try:
            subscribe_tickers()
            quotes_maker.start_symbol_ticker_socket(symbol, process_ticker)
            logger.debug(f'{symbol} subscribed')
            pass
        except Exception as e:
            traceback.print_exc()
            stacktrace = format_stacktrace()
            logger.error(f'ERROR не удалось подписаться на пару "{symbol}": {e} {stacktrace}')
            return

        logger.debug(f'SUBSCRIBED to {symbol.capitalize()}')
    else:
        logger.debug(f'{symbol} alredy subscriped')


def process_signal(signal_id) -> None:
    logger.debug("Processing signal")
    # message = "Processing signal\n";
    # logger.info(message, notify=NotifyType.Signal)
    connect()
    ...
    if not query.exists():
        message = f'No strategies for: {signal.bot} | Signal #{signal_id}'
        # logger.info(message, signal=signal)
        return

    message = 'Found %s strategy(ies) for this signal\n' % len(query)
    # logger.info(message, notify=NotifyType.Signal)
    for strategy in query:
        message += 'Strategy %s\n' % strategy
        message += f'Bot>> {signal.bot} | strategy {strategy} | simbol "{signal.symbol}" | price {cfloat(signal.price)}?\n'
        # logger.info(message, notify=NotifyType.Signal, strategy=strategy)
        if signal.price < strategy.satoshi_filter * 1e-8:
            message += f'цена пары меньше допустимой {signal.price} <= {cfloat(strategy.satoshi_filter * 1e-8)}'
        ...
        if signal.symbol.daily_volume < strategy.daily_limit_filter:
            message += f'дневной объём пары меньше допустимой {signal.symbol.daily_volume} < {strategy.daily_limit_filter}'
            # logger.debug(message)
            logger.info(message, signal=signal)
            continue
        if not strategy.frozen:
            accounts = strategy.assigned_accounts.all()
            if not accounts.exists():
                message += f"No assigned accounts. Quit."
                # logger.debug(message)
                logger.info(message, signal=signal)
            for account in accounts:
                if account.id in clients:
                    message += f"Trying to open order."
                    # logger.debug(message)
                    logger.info(message, notify=NotifyType.Signal, strategy=strategy)
                    clients[account.id].open_order(signal.symbol, signal.bot, strategy, signal.price, signal)
                else:
                    message += f"аккаунт {account.name} не подключен"
                    # logger.debug(message)
                    logger.info(message, signal=signal)
        else:
            message += f'strategy is frozen...'
            # logger.debug(message)
            logger.info(message, signal=signal)
        ...
        if strategy.guard_enabled and strategy.guard_test_account_id and strategy.frozen:
            if strategy.guard_test_account_id in clients:
                message = message + f'Openning test order...'
                logger.debug(message)
                logger.info(message, notify=NotifyType.Signal, strategy=strategy)
                clients[strategy.guard_test_account_id].open_order(signal.symbol, signal.bot, strategy, signal.price,
                                                                   signal)
            else:
                message = message + f'test аккаунт {account.name} не подключен'
                logger.debug(message)
                logger.info(message, signal=signal)
...
def process_smart_order(order_id) -> None:
    logger.debug("Processing a smart oder")
